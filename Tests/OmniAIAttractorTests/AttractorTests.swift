...
final class AttractorTests: XCTestCase {
    ...
    func testWaitHumanSkipReturnsDefault() async throws {
        let dot = """
        digraph pipeline {
            start [shape=Mdiamond]
            ask [shape=hexagon, prompt="Pick one"]
            pathA [shape=box]
        ...
        let outcome = try await handler.execute(node: askNode, context: ctx, graph: graph, logsRoot: logsRoot)
        XCTAssertEqual(outcome.status, .success)
        XCTAssertEqual(outcome.preferredLabel, "default")
    }
    ...
    func testEngineAllowPartialAcceptsPartialSuccess() async throws {
        // Backend returns FAIL, node has allow_partial=true and max_retries=1
        // After 1 retry (2 total attempts), both fail, allow_partial kicks in
        let backend = MockCodergenBackend(result: CodergenResult(
            response: "failed",
            status: .fail,
            notes: "persistent failure"
        ))
        let dot = """
        digraph test {
            start [shape=Mdiamond]
        ...
        // allow_partial converts the final FAIL to PARTIAL_SUCCESS
        XCTAssertEqual(result.nodeOutcomes["task"], .partialSuccess,
                       "allow_partial=true should convert exhausted FAIL to PARTIAL_SUCCESS")
        XCTAssertEqual(result.status, .success,
