...
@Test func adwaitaRendererHasLargeListAndTextFastPaths() throws {
    ...
    #expect(renderer.contains("omni_adw_node_append(scrollNode, list)"))
    #expect(renderer.contains("func firstButtonActionID(in node: SemanticNode) -> Int?"))
    #expect(renderer.contains("rows.append((label: label, actionID: firstButtonActionID(in: node)))"))
...
@Test func adwaitaRendererPreservesDialogOverlayStyling() throws {
    ...
    #expect(shim.contains("adw_dialog_close(app->modal_dialog)"))
...
@Test func iGopherAdwaitaParityFixesStayCovered() throws {
    ...
    #expect(shim.contains(".omni-plain-list row { min-height: 22px; padding: 0 0; background: transparent; }"))
