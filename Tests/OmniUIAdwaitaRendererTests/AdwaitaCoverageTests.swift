...
@Test func adwaitaRendererConsumesRendererNeutralSemanticLayer() throws {
    ...
    #expect(renderer.contains("let hadActiveAnimations = runtime._hasActiveAnimations"))
    #expect(renderer.contains("_ = runtime._tickAnimations()"))
    #expect(renderer.contains("let invalidationReason = runtime.renderInvalidationReason(size: renderSize)"))
    #expect(renderer.contains("if hadActiveAnimations || invalidationReason != nil"))
...
@Test func adwaitaSidebarAndToolbarPolishUsesGenericNativeBehavior() throws {
    ...
    #expect(renderer.contains("omni_adw_symbol_image_new(text, fallback, fallback)"))
    #expect(header.contains("omni_adw_symbol_image_new"))
    ...
    #expect(shim.contains("omni_symbolic_icon_name_for_system_name"))
    #expect(shim.contains("omni_known_resource_symbolic_icon"))
    #expect(shim.contains("omni_available_symbolic_icon"))
    #expect(shim.contains("gtk_icon_theme_has_icon"))
    #expect(shim.contains("GTK_IS_LABEL(widget) || GTK_IS_BUTTON(widget) || GTK_IS_IMAGE(widget)"))
    #expect(shim.contains("omni-symbol-image"))
...
@Test func adwaitaSceneInitializerInstallsNativeSettingsAndCommandsChrome() throws {
    ...
    #expect(renderer.contains("if box.previousSettingsRoot == nil,"))
    #expect(renderer.contains("invokeAdwaitaSettingsFallback(box: box)"))
    #expect(renderer.contains("for label in [\"Settings\", \"Preferences\"]"))
    #expect(renderer.contains("syncNativeHeaderEntry(presentation.toolbar.entry, app: cApp)"))
    #expect(renderer.contains("AdwaitaHeaderEntry.extract(from:"))
    #expect(renderer.contains("extractInlineToolbar(from:"))
    #expect(renderer.contains("AdwaitaSemanticDumper.dumpIfRequested(snapshot.root, section: \"RAW\")"))
    #expect(renderer.contains("headerLabel(forAccessibilityIdentifier:"))
    #expect(renderer.contains("case \"home-button\":"))
    #expect(renderer.contains("case \"settings-button\":"))
    #expect(renderer.contains("!headerActions(in: [node.children[split.index]], placement: .end).isEmpty"))
...
@Test func adwaitaRendererHasLargeListAndTextFastPaths() throws {
    ...
    #expect(shim.contains("G_DEFINE_TYPE_WITH_CODE"))
    #expect(shim.contains("G_IMPLEMENT_INTERFACE(G_TYPE_LIST_MODEL"))
    #expect(shim.contains("omni_list_model_get_item"))
    #expect(shim.contains("omni_list_row_object_new"))
    #expect(shim.contains("g_list_model_items_changed"))
    #expect(shim.contains("OMNI_NATIVE_ACCESSIBILITY_ROW_UPDATE_LIMIT"))
    #expect(shim.contains("omni_accessible_label_with_native_update"))
    #expect(shim.contains("if (size > OMNI_NATIVE_ACCESSIBILITY_ROW_UPDATE_LIMIT) return;"))
    #expect(shim.contains("g_object_set_data(G_OBJECT(widget), \"omni-accessible-pos-cache\""))
    ...
    #expect(shim.contains("g_object_set_data_full(G_OBJECT(node->widget), \"omni-list-model\", model, g_object_unref)"))
...
@Test func adwaitaRendererPreservesDialogOverlayStyling() throws {
    ...
    #expect(renderer.contains("shouldPresentModalInSettingsWindow"))
    #expect(renderer.contains("omni_adw_app_set_settings(app, node)"))
    ...
    #expect(shim.contains("adw_dialog_set_can_close(dialog, TRUE)"))
    #expect(shim.contains("adw_dialog_set_content_width(dialog, 520)"))
    #expect(shim.contains("widget_contains_scrolled_window"))
    #expect(shim.contains("adw_dialog_set_content_height(dialog, 560)"))
    #expect(shim.contains("omni-sheet-surface"))
    #expect(shim.contains("@popover_bg_color"))
    #expect(shim.contains("box-shadow: 0 12px 36px"))
    ...
    #expect(shim.contains("adw_dialog_set_child(dialog, surface)"))
    ...
    #expect(shim.contains("adw_alert_dialog_set_extra_child(alert, surface)"))
    #expect(shim.contains("background: @window_bg_color"))
...
@Test func adwaitaDocumentsLiquidGlassAndCRTApproximationPath() throws {
    ...
    #expect(renderer.contains("case .shadow(let color, let radius, let x, let y):"))
    #expect(renderer.contains("shadowCSSClass(color: color, radius: radius, x: x, y: y)"))
    #expect(renderer.contains("case .crt, .accessibilityLabel, .noOp:"))
    #expect(renderer.contains("crtScanlineOverlayCSSClass(from: node.children)"))
    #expect(renderer.contains("crtScanlineOverlayRun(in: renderChildren"))
    #expect(renderer.contains("repeating-linear-gradient(to bottom"))
    ...
    #expect(shim.contains("omni-crt-overlay"))
    #expect(shim.contains("cairo_pattern_create_radial"))
    #expect(shim.contains("gtk_widget_set_can_target(node->widget, FALSE)"))
    #expect(supportDoc.contains("Liquid Glass and CRT effect modifiers are semantic metadata in the Adwaita backend"))
    #expect(supportDoc.contains("CRT scanlines, vignette gradients, and glow shadows lower to pass-through drawing/CSS overlays"))
    #expect(supportDoc.contains("Path, shapes, and gradients lower to semantic drawing islands"))
}
...
@Test func adwaitaMapsDrawingSurfacePrimitivesToNativeDrawingIslands() throws {
    ...
    #expect(semantic.contains("case .gradient(let gradient):"))
    #expect(semantic.contains("kind: .drawingIsland(.gradient("))
    #expect(semantic.contains("startX: points.startX"))
    #expect(renderer.contains("case .drawingIsland(let kind):"))
    #expect(renderer.contains("omni_adw_drawing_new(\"OmniUI shape(\\(name))\", fill)"))
    #expect(renderer.contains("omni_adw_gradient_new("))
    #expect(renderer.contains("gradientCSSClass(colors:"))
    #expect(renderer.contains("linear-gradient(\\(direction), \\(cssStops.joined(separator: \", \")))"))
    #expect(renderer.contains("background-image: \\(gradient)"))
    #expect(shim.contains("gtk_drawing_area_new()"))
    #expect(shim.contains("gtk_widget_set_can_target(node->widget, FALSE)"))
    #expect(shim.contains("gtk_drawing_area_set_draw_func(GTK_DRAWING_AREA(node->widget), omni_adw_gradient_draw"))
    #expect(shim.contains("omni_widget_set_layout_size_recursive(base_child, child_width, child_height)"))
    #expect(shim.contains("gtk_widget_get_size_request(node->widget, &current_width, &current_height)"))
...
@Test func cAdwaitaShimExportsGtkWidgetsToMacOSAccessibility() throws {
    ...
    #expect(shim.contains("OMNI_MACOS_ACCESSIBILITY_MAX_ELEMENTS"))
    #expect(shim.contains("omni_macos_accessibility_has_exported_row_ancestor"))
    #expect(shim.contains("g_timeout_source_new(350)"))
    #expect(shim.contains("if (app->macos_accessibility_sync_source != 0) return;"))
...
@Test func adwaitaRendererCanDumpMainSettingsAndCommandSemanticTrees() throws {
    ...
    #expect(renderer.contains("let accessibleLabel = AdwaitaReconciliation.accessibleLabel(for: node).trimmingCharacters(in: .whitespacesAndNewlines)"))
...
@Test func adwaitaRendererPropagatesPreferredColorSchemeToNativeAdwaita() throws {
    let renderer = try readRepositoryFile("Sources/OmniUIAdwaitaRenderer/AdwaitaRenderer.swift")
    let linuxCompat = try readRepositoryFile("Sources/OmniUICore/AppKitLinuxCompat.swift")
    ...
    #expect(renderer.contains("switch scheme ?? currentAppearance ?? environmentColorSchemeOverride()"))
    #expect(renderer.contains("return \"light\""))
    #expect(renderer.contains("return \"dark\""))
    #expect(renderer.contains("return \"system\""))
    #expect(renderer.contains("nativeScheme.withCString { omni_adw_set_color_scheme($0) }"))
    #expect(linuxCompat.contains("OMNIUI_ADWAITA_COLOR_SCHEME"))
    #expect(linuxCompat.contains("OMNIUI_COLOR_SCHEME"))
    #expect(linuxCompat.contains("GTK_THEME"))
    #expect(linuxCompat.contains("_omniCurrentAppearanceColorScheme() ?? .light"))
    #expect(linuxCompat.contains("scheme ?? Self.colorScheme(for: appearanceName) ?? Self.environmentColorSchemeOverride()"))
}
...
@Test func iGopherAdwaitaParityFixesStayCovered() throws {
    ...
    for toolbar in [omniToolbar, adwaitaToolbar] {
        ...
        #expect(toolbar.contains("Button(action: { showPreferences = true })"))
        #expect(toolbar.contains(".keyboardShortcut(\",\", modifiers: [.command])"))
        #expect(toolbar.contains(".keyboardShortcut(\"[\", modifiers: [.command])"))
        #expect(toolbar.contains(".keyboardShortcut(\"]\", modifiers: [.command])"))
...
@Test func adwaitaRendererCoversNativeSettingsWidgets() throws {
    ...
    #expect(renderer.contains("case settings"))
    #expect(renderer.contains("switchStyledRow(in:"))
    #expect(renderer.contains("if let switchRow = switchStyledRow(in: node)"))
    #expect(renderer.contains("context == .settings"))
    #expect(renderer.contains("buildSettingsRow(child, context: .settings)"))
    #expect(renderer.contains("text.contains(\"[━━●]\")"))
    #expect(renderer.contains("text.contains(\"[○━━]\")"))
    #expect(renderer.contains("title.hasPrefix(\"> \")"))
    ...
    #expect(shim.contains("control && control->button"))
    #expect(shim.contains("gtk_widget_set_tooltip_text(button, has_label ? label : \"Color\")"))
    #expect(shim.contains("gtk_widget_set_size_request(scale, 180, -1)"))
    #expect(shim.contains("gtk_widget_set_tooltip_text(swatch_button, names[i])"))
