...
enum _DebugLayout {
    ...
    static func layout(node: _VNode, in rect: _Rect) -> Result {
        ...
        _ = draw(node: node, origin: rect.origin, maxSize: rect.size, canvas: &canvas, hitRegions: &hits)
    ...
    private static func draw(
        ...
        case .group(let nodes):
            ...
            for n in nodes {
                ...
                let s = draw(node: n, origin: origin, maxSize: maxSize, canvas: &canvas, hitRegions: &hitRegions)
        ...
        case .padding(let amount, let child):
            ...
            let s = draw(node: child, origin: inner.origin, maxSize: inner.size, canvas: &canvas, hitRegions: &hitRegions)
        ...
        case .stack(let axis, let spacing, let children):
            ...
            for (idx, child) in children.enumerated() {
                ...
                let s = draw(node: child, origin: cursor, maxSize: remaining, canvas: &canvas, hitRegions: &hitRegions)
                switch axis {
                case .vertical:
                    cursor.y += s.height
        ...
        case .button(let id, let isFocused, let label):
            ...
            let labelSize = draw(node: label, origin: labelOrigin, maxSize: labelMax, canvas: &canvas, hitRegions: &hitRegions)
        ...
        case .menu(let id, let isFocused, let isExpanded, let title, let value, let items):
            ...
            // Top border
            let topY = origin.y + 1
            if topY < origin.y + maxSize.height {
                put("+", at: _Point(x: origin.x, y: topY), canvas: &canvas)
                for i in 0..<boxInnerWidth { put("-", at: _Point(x: origin.x + 1 + i, y: topY), canvas: &canvas) }
                put("+", at: _Point(x: origin.x + boxInnerWidth + 1, y: topY), canvas: &canvas)
            }

            var usedHeight = 2
            for item in items {
                let y = origin.y + usedHeight
                if y >= origin.y + maxSize.height - 1 { break }
                put("|", at: _Point(x: origin.x, y: y), canvas: &canvas)
                for i in 0..<boxInnerWidth { put(" ", at: _Point(x: origin.x + 1 + i, y: y), canvas: &canvas) }
                put("|", at: _Point(x: origin.x + boxInnerWidth + 1, y: y), canvas: &canvas)
                ...
                for (i, ch) in prefix.enumerated() {
                    put(String(ch), at: _Point(x: origin.x + 1 + i, y: y), canvas: &canvas)
                }
                let labelClipped = String(item.label.prefix(max(0, boxInnerWidth - prefix.count)))
                for (i, ch) in labelClipped.enumerated() {
                    put(String(ch), at: _Point(x: origin.x + 1 + prefix.count + i, y: y), canvas: &canvas)
                }

                let optRect = _Rect(origin: _Point(x: origin.x, y: y), size: _Size(width: boxWidth, height: 1))
            ...
            // Bottom border
            let bottomY = origin.y + usedHeight
            if bottomY < origin.y + maxSize.height {
                put("+", at: _Point(x: origin.x, y: bottomY), canvas: &canvas)
                for i in 0..<boxInnerWidth { put("-", at: _Point(x: origin.x + 1 + i, y: bottomY), canvas: &canvas) }
                put("+", at: _Point(x: origin.x + boxInnerWidth + 1, y: bottomY), canvas: &canvas)
                usedHeight += 1
            }

            return _Size(width: max(headWidth, boxWidth), height: usedHeight)
