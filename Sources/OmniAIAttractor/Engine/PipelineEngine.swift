...
public final class PipelineEngine: @unchecked Sendable {
    ...
    public func run(graph: Graph) async throws -> PipelineResult {
        ...
        let state = ExecutionState(graph: g, context: context, logsRoot: config.logsRoot)

        try initializeRun(state: state)
    ...
    private func initializeRun(state: ExecutionState) throws {
            ...
            if outcome.status == .fail {
                ...
                if let edge = selectNextEdge(from: currentId, outcome: outcome, context: state.context, graph: state.graph) {
                    ...
                    if edge.loopRestart {
                        ...
                        // Clear completed nodes and retry counts (but keep context updates)
                        state.completedNodes.removeAll()
                        state.nodeRetries.removeAll()
                        state.nodeOutcomes.removeAll()

                        // Create a fresh log subdirectory for this cycle and update state
                        let cycleDir = state.logsRoot
                            .deletingLastPathComponent()
                            .appendingPathComponent(
                                state.logsRoot.lastPathComponent + "_cycle\(loopCount)"
                            )
                        try? FileManager.default.createDirectory(
                            at: cycleDir,
                            withIntermediateDirectories: true
                        )
                        state.logsRoot = cycleDir
                    }
                    state.currentNodeId = edge.to
                } else {
    ...
    private func executeWithRetry(node: Node, state: ExecutionState) async throws -> Outcome {
        ...
        while true {
            ...
            // Retry or fail: check retry budget
            let currentRetries = state.nodeRetries[node.id] ?? 0
            if (outcome.status == .retry || outcome.status == .fail) && currentRetries < maxRetries && node.maxRetries > 0 {
            ...
            // Retries exhausted: check allow_partial
            // If allow_partial=true and retries are exhausted, accept PARTIAL_SUCCESS
            // instead of the final FAIL/RETRY status. Spec §2.6.
            if node.allowPartial && (outcome.status == .fail || outcome.status == .retry) {
                return Outcome(
...
private final class ExecutionState {
    ...
    init(graph: Graph, context: PipelineContext, logsRoot: URL) {
        self.graph = graph
        self.context = context
        self.logsRoot = logsRoot
    }
